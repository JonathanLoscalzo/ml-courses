...
#sum(mat,2) suma por filas
#sum(mat,1) suma por columnas
# sum(mat(:)) suma todos los elementos
l1 = sum((Theta1(:, 2:end).^2)(:));
l2 = sum((Theta2(:, 2:end).^2)(:));
J = (1/m) * sum(sum((-Y).*log(h) - (1-Y).*log(1-h), 2)) + ((lambda/(2*m)) * (l1+l2));
